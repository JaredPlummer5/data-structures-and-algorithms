...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    result.push(
      input[i].filter(element => typeof element === 'number' && element % 5 === 0)
...
let findMaleAndFemale = (data) => {
  const filteredData = data.filter(elem => elem.gender === 'male' || elem.gender === 'female');
  const characterNames = filteredData.map(elem => elem.name);
  const result = characterNames.join(' and ');
  return result;
};
...
let findShortest = (data) => {
  const shortestCharacter = data.reduce((a, b) => {
