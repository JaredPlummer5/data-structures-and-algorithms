...
const screenForNames = (arr) => {
  ...
  let prefixArray = arr.filter((element) => regex.test(element));
  return prefixArray;
};
...
const toTitleCase = (arr) => {
  // Solution code here...
  let titleCase = arr.map(element => {
    return element.replace(/^[a-z]/, element[0].toUpperCase());
  });
  return titleCase;
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let biggerThanLukeArray = arr.filter(element => {
  ...
  let namesStr = biggerThanLukeArray.map(element => element.name);
...
const sortBy = (property, arr) => {
  // Solution code here...
  let sortByPropertyArray = arr.sort((a, b) => {
